...
class ResolveOrderFinancialEvidence
{
    ...
    public function execute(string $orderUuid): OrderFinancialResolution
    {
        ...
        $keys = $preparations->map(fn ($p) => [$p->canonical_account_key, $p->environment])
            ->unique(fn ($pair) => implode(':', $pair))->sortBy(fn ($pair) => implode(':', $pair))->values()->all();

        return DB::transaction(function () use ($identity, $keys): OrderFinancialResolution {
            $scopes = [];
            ...
            if ($intent->currency !== $order->currency || PaymentOperation::where('payment_intent_id', $intent->id)
                ->where('state', 'uncertain')->lockForUpdate()->exists()) {
