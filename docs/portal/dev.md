...
| `GET /patient/health/series` | patient | The Health screen in one call: `{period, series: {weight, blood_pressure, heart_rate, blood_glucose}, goal: {weight, date}, items, truncated, readings_considered}`. `period` `1year` (default) or `all`. Built from the vitals **list** (up to 1000 newest readings) plus goals — not `/vitals/trends`, which caps at a year, keeps only dates and computes loss from two rows. Series oldest first with exact instants; blood pressure only when both halves exist; `items` newest 50, not period-scoped, filtered by the `vitals` spec. Nothing stored. |
