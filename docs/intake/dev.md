...
## Anonymous patient registration identity boundary (2026-09-06, reworked 2026-09-13)

Registration creates **no account at all**. `POST /patient/auth/register` takes only an address, answers a uniform 202, and a queued job emails a single-use link: a create-account link when the address has a claimable order (unclaimed, with an encounter carrying a chart id our server recorded), a password-reset link when it already has an account, nothing otherwise. The account is created only when that link is used (`POST /patient/auth/create-account`), with the email taken from the address the link was delivered to, verified, and linked to the order's chart in one transaction. Nothing anonymous ever calls `findPatientByEmail`, and an exact email match is still insufficient on its own — the encounter and the mailbox are both required. See `docs/portal/dev.md` → "Accounts are created by the link". `AccountLinkRequestTest` and `CreateAccountTest` pin it.
