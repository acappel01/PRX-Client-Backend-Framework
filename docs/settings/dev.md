...
**The brand inside the body — `MailBrand`.** Laravel's stock markdown layout (`<title>`, header, footer), its header link, and the notification template's "Regards," all read `config('app.name')` / `config('app.url')`. On this backend those are `APP_NAME` (the admin product) and the admin hostname, and no admin setting replaced them, so the first live plan email called itself "PRX Backend" and linked the admin login. `APP_NAME` must NOT be changed per brand to fix it: session, cache, Redis and Horizon prefixes and the prescribe-rx idempotency key are derived from it. Instead `resources/views/vendor/mail/html/{layout,message}.blade.php`, `vendor/mail/text/message.blade.php` and `vendor/notifications/email.blade.php` are copies of the framework views that call `App\Services\Mail\MailBrand` — name: `BrandSettings::name` → `mail.from.name`; link: `BrandSettings::site_url` → `cms.frontend.url` → no link. Mail components resolve from `resources/views/vendor/mail` via `mail.markdown.paths` — the framework's base config already sets it and is merged under ours; `config/mail.php` repeats it explicitly. **An app-level `markdown` block without `paths` would shadow the base and silently drop the overrides.** Notification views load from `vendor/notifications` automatically. On a Laravel upgrade, diff these four copies against the framework's, plus the unlinked header row in `html/message.blade.php`, hand-copied from `header.blade.php`. **Horizon must be restarted after deploying view overrides** — the view finder caches resolved paths per process. The header link (`MailBrand::url`: site URL → `CMS_FRONTEND_URL`) and the plan button (`SendPlanEmail::planUrl`: `CMS_FRONTEND_URL` → `APP_URL`) are separate chains on purpose; see both docblocks. `MailBrandingTest` renders HTML and text for real.
...
default 30, 5–240), `session_max_hours` (int, default 12, 1–720), `two_factor_policy` (string `off|optional|required`, default
`off`, read through `PortalSettings::twoFactorPolicy()`, which falls back to `Off` on an unknown value, and published
in `/config` as `portal.two_factor.policy`), `trusted_device_days` (int 0–90, default 30; `/config`
`portal.two_factor.trusted_device_days`). The two session keys are enforced by
